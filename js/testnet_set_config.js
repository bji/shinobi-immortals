...
(async () => {
    ...
    var transaction = testnet_set_config({ });
 ...
 * testnet_set_config
 *
 * Sets the testnet nifty stakes program config.  The admin user pubkey will be {admin_user_pubkey}, and the metadata_program_id will be {metadata_program_id} (temporary value).
 *
...
**/
function testnet_set_config(args)
{
  let tmp;
  let keys;
  let data;
  ...
  tmp = "ATPQjic4Jd8ra1hcXBNPp7MinWr9CmdcmFjSZrQ9ZZUa";
  solxact_push_pubkey(tmp, data);
